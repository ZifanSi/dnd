// Entry point: wire the data, per-city 3D overrides and the 3D map viewer together.
...
const markers = createMarkers(viewer.scene, container, entries, { onClick: flyToEntry });
...
viewer.onResize((w, h) => markers.resize(w, h));
...
function restoreFromHash() {
  ...
  const entry = value && byName.get(value.toLowerCase());
  if (key === "place" && entry) {
